...
export function generateOHLCV(
  ...
  for (let i = 0; i < days; i++) {
    ...
    const high =
      Math.max(open, close, intraday1, intraday2) * (1 + Math.abs(normalRandom()) * 0.005);
    const low =
      Math.min(open, close, intraday1, intraday2) * (1 - Math.abs(normalRandom()) * 0.005);
...
function normalRandom(): number {
  return (
    Math.sqrt(-2 * Math.log(Math.max(Math.random(), 1e-9))) * Math.cos(2 * Math.PI * Math.random())
  );
}
