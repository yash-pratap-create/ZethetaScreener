...
> = {
  '1D': { label: '1 Day', bars: 390, interval: 60 },
  '1W': { label: '1 Week', bars: 250, interval: 1800 },
  '1M': { label: '1 Month', bars: 220, interval: 7200 },
  '3M': { label: '3 Months', bars: 250, interval: 86400 },
  '6M': { label: '6 Months', bars: 130, interval: 86400 * 2 },
  '1Y': { label: '1 Year', bars: 252, interval: 86400 },
};
function generateCandlestickData(
  ...
  for (let i = 0; i < count; i++) {
    ...
    const priceChange = currentPrice * (0.0008 + shock * 0.006);
...
export function SymbolPageClient({ stock: baseStock }: SymbolPageClientProps) {
  ...
  const { data: fundamentals, isLoading: funLoading } = useQuery<{
    success: boolean;
    data: FundDetails;
  }>({
  ...
  const drawVolumeProfile = useCallback(
    (bars: OHLCVBar[]) => {
      ...
      bars.forEach((b) => {
        const idx = Math.min(bucketsCount - 1, Math.floor((b.close - minP) / bucketSize));
      ...
      for (let i = 0; i < bucketsCount; i++) {
        ...
        const barWidth = (b.total / maxVol) * (canvas.width * 0.25);
        const xStart = 0;
    ...
    (bars: OHLCVBar[]) => {
      ...
      if (activeIndicators.ema20 && seriesRefs.current.ema20) {
        ...
        seriesRefs.current.ema20.setData(
          emaData.map((d) => ({ time: toTime(d.time), value: d.value })),
        );
      ...
      if (activeIndicators.sma50 && seriesRefs.current.sma50) {
        ...
        seriesRefs.current.sma50.setData(
          sma50Data.map((d) => ({ time: toTime(d.time), value: d.value })),
        );
      ...
      if (activeIndicators.sma200 && seriesRefs.current.sma200) {
        ...
        seriesRefs.current.sma200.setData(
          sma200Data.map((d) => ({ time: toTime(d.time), value: d.value })),
        );
      ...
      if (
        ...
        seriesRefs.current.bbUpper &&
        seriesRefs.current.bbMiddle &&
        seriesRefs.current.bbLower
      ) {
        const bbData = calculateBollingerBands(bars, 20, 2);
        seriesRefs.current.bbUpper.setData(
          bbData.map((d) => ({ time: toTime(d.time), value: d.upper })),
        );
        seriesRefs.current.bbMiddle.setData(
          bbData.map((d) => ({ time: toTime(d.time), value: d.middle })),
        );
        seriesRefs.current.bbLower.setData(
          bbData.map((d) => ({ time: toTime(d.time), value: d.lower })),
        );
      } else {
      ...
      if (seriesRefs.current.rsi) {
        ...
        seriesRefs.current.rsi.setData(
          rsiPoints.map((d) => ({ time: toTime(d.time), value: d.value })),
        );
      }
  ...
  useEffect(() => {
    ...
    seriesRefs.current.sma50 = chart.addSeries(LineSeries, {
      color: '#3b82f6',
      lineWidth: 2,
      title: 'SMA(50)',
    });
    seriesRefs.current.sma200 = chart.addSeries(LineSeries, {
      color: '#a855f7',
      lineWidth: 2,
      title: 'SMA(200)',
    });
    ...
    const resizeObserver = new ResizeObserver(() => {
      if (mainRef.current && mainChart.current)
        mainChart.current.resize(mainRef.current.clientWidth, mainRef.current.clientHeight);
      if (rsiRef.current && rsiChart.current)
        rsiChart.current.resize(rsiRef.current.clientWidth, rsiRef.current.clientHeight);
  ...
  const activeCandle =
    focusedIndex !== null && barsData[focusedIndex] ? barsData[focusedIndex] : null;
  ...
  const getFlashStyle = () => {
    if (!flash) return {};
    const color = flash.direction === 'up' ? 'rgba(34, 197, 94, 0.22)' : 'rgba(239, 68, 68, 0.22)';
  ...
  return (
    <div className="flex-1 flex flex-col p-4 md:p-6 overflow-y-auto space-y-6">
      ...
      <div
        className="flex flex-col lg:flex-row justify-between items-start lg:items-center p-6 rounded-xl border border-border bg-surface-2 gap-6"
        style={getFlashStyle()}
      >
        <div className="flex flex-col gap-2">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="text-xs px-2 py-0.5 rounded bg-surface-3 border border-border text-text-secondary font-medium">
              {stock.sector}
            </span>
            <span className="text-xs px-2 py-0.5 rounded bg-accent-primary/10 border border-accent-primary/20 text-accent-primary font-medium">
              {stock.marketCapCategory}
            </span>
          </div>
          <p className="text-sm text-text-secondary font-medium">
            {stock.companyName} • <span className="text-text-muted">{stock.industry}</span>
          </p>
        ...
        <div className="flex flex-col items-start lg:items-end gap-1.5">
          <div className="flex items-baseline gap-2">
            <span className="text-3xl font-bold font-mono tracking-tight text-text-primary">
              ₹
              {stock.lastPrice.toLocaleString(undefined, {
                minimumFractionDigits: 2,
                maximumFractionDigits: 2,
              })}
            </span>
            <span
              className={`text-sm font-semibold font-mono flex items-center ${isUp ? 'text-green-400' : 'text-red-400'}`}
            >
              {isUp ? '▲' : '▼'} {Math.abs(diffAbsolute).toFixed(2)} ({isUp ? '+' : ''}
              {diffPercent.toFixed(2)}%)
            </span>
          </div>
          <div className="flex items-center gap-1.5 text-[10px] text-text-muted uppercase font-bold">
            <span
              className={`w-2 h-2 rounded-full ${connStatus === 'connected' ? 'bg-green-400' : 'bg-yellow-400'}`}
            />
      ...
      <div className="grid grid-cols-1 xl:grid-cols-4 gap-6">
        <div
          className="xl:col-span-3 flex flex-col p-4 rounded-xl border border-border bg-surface-2 space-y-4"
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 bg-surface-3 px-3 py-2 rounded-lg text-xs font-mono border border-border/60">
            <span className="text-text-muted uppercase text-[9px] font-bold font-sans">
              Metrics:
            </span>
            <span className="text-text-secondary">
              O: <strong className="text-text-primary">{displayInfo.o?.toFixed(2)}</strong>
            </span>
            <span className="text-text-secondary">
              H: <strong className="text-green-400">{displayInfo.h?.toFixed(2)}</strong>
            </span>
            <span className="text-text-secondary">
              L: <strong className="text-red-400">{displayInfo.l?.toFixed(2)}</strong>
            </span>
            <span className="text-text-secondary">
              C:{' '}
              <strong className="text-text-primary" style={{ fontWeight: 700 }}>
                {displayInfo.c?.toFixed(2)}
              </strong>
            </span>
            <span className="text-text-secondary">
              V:{' '}
              <strong className="text-text-primary">{(displayInfo.v ?? 0).toLocaleString()}</strong>
            </span>
            {activeCandle && (
              <span className="text-[10px] text-accent-primary bg-accent-primary/5 px-1.5 py-0.5 rounded uppercase font-bold font-sans ml-auto">
                Keyboard Navigating
              </span>
            )}
          </div>
          ...
          {showTable ? (
              ...
              <table className="w-full text-left border-collapse text-xs">
                ...
                <tbody>
                  {barsData.map((bar, idx) => (
                    <tr
                      key={idx}
                      className="border-b hover:bg-hover/10"
                      style={{ borderColor: 'var(--color-border)' }}
                    >
                      <td className="py-2 px-3 font-mono">
                        {new Date(bar.time * 1000).toLocaleDateString()}{' '}
                      ...
                      <td
                        className="py-2 text-green-400 font-mono"
                        style={{ color: highContrastMode ? '#00ffff' : '#22c55e' }}
                      >
                        {bar.high.toFixed(2)}
                      </td>
                      <td
                        className="py-2 text-red-400 font-mono"
                        style={{ color: highContrastMode ? '#ff00ff' : '#ef4444' }}
                      >
                        {bar.low.toFixed(2)}
                      </td>
                      <td
            ...
            <div className="flex-1 flex flex-col space-y-4 min-h-[340px]">
              ...
              <div className="flex-[1] relative border border-border/40 rounded-lg overflow-hidden min-h-[90px]">
                ...
                <div
                  className="absolute top-2 left-2 text-[9px] uppercase tracking-wider font-bold bg-surface-2/80 px-1.5 py-0.5 rounded text-accent-secondary"
                  style={{ zIndex: 5, color: '#fb7185' }}
                >
                  RSI (14)
                </div>
              </div>
        ...
        <div className="xl:col-span-1 p-5 rounded-xl border border-border bg-surface-2 flex flex-col space-y-4">
          <h2 className="text-sm font-bold uppercase tracking-wider text-text-primary border-b border-border pb-2">
            Technical Stats
          </h2>
          <div className="grid grid-cols-2 xl:grid-cols-1 gap-4 text-xs">
            ...
            <div className="flex justify-between border-b border-border/40 pb-2 xl:pb-1">
              ...
              <span
                className={`font-semibold font-mono ${stock.macdSignal === 'Bullish' ? 'text-green-400' : stock.macdSignal === 'Bearish' ? 'text-red-400' : 'text-text-secondary'}`}
              >
                {stock.macdSignal}
              </span>
            </div>
          ...
          <h2 className="text-sm font-bold uppercase tracking-wider text-text-primary border-b border-border pb-2 pt-2">
            Fundamentals
          </h2>
          <div className="grid grid-cols-2 xl:grid-cols-1 gap-4 text-xs">
            <div className="flex justify-between border-b border-border/40 pb-2 xl:pb-1">
            ...
            <div className="flex justify-between border-b border-border/40 pb-2 xl:pb-1">
              ...
              <span className="font-mono font-bold text-text-primary">
                ₹{stock.freeCashFlow.toLocaleString()}
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="p-6 rounded-xl border border-border bg-surface-2 space-y-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 text-xs">
          <div className="bg-surface-3 p-3 rounded-lg border border-border/50">
            ...
            <p className="text-md font-extrabold font-mono text-text-primary mt-1">
              {stock.roe.toFixed(2)}%
            </p>
          </div>
          <div className="bg-surface-3 p-3 rounded-lg border border-border/50">
          ...
          <div className="bg-surface-3 p-3 rounded-lg border border-border/50">
            ...
            <p className="text-md font-extrabold font-mono text-text-primary mt-1">
              {stock.evEbitda.toFixed(2)}
            </p>
          </div>
          <div className="bg-surface-3 p-3 rounded-lg border border-border/50">
            <p className="text-[10px] text-text-muted uppercase font-bold">Revenue Growth YoY</p>
            <p className="text-md font-extrabold font-mono text-green-400 mt-1">
        ...
        {funLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="border border-border/60 rounded-xl bg-surface-3 p-4">
              <h3 className="text-xs font-bold uppercase text-accent-primary mb-3">
                Balance Sheets Statement (₹ Cr)
              </h3>
              <table className="w-full text-left text-xs border-collapse font-mono">
                <thead>
                  <tr className="border-b border-border text-[10px] text-text-muted uppercase font-bold">
                    ...
                    {fundamentals.data.balanceSheet.map((bs) => (
                      <th key={bs.year} className="text-right py-2">
                        {bs.year}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  <tr className="border-b border-border/40">
                    ...
                    {fundamentals.data.balanceSheet.map((bs) => (
                      <td key={bs.year} className="text-right text-text-primary">
                  ...
                  <tr className="border-b border-border/40">
                    ...
                    {fundamentals.data.balanceSheet.map((bs) => (
                      <td key={bs.year} className="text-right text-green-400 font-extrabold">
                        {bs.shareholdersEquity.toLocaleString()}
                      </td>
                    ))}
                  </tr>
                </tbody>
              ...
              <table className="w-full text-left text-xs border-collapse font-mono">
                <thead>
                  <tr className="border-b border-border text-[10px] text-text-muted uppercase font-bold">
                    ...
                    {fundamentals.data.incomeStatement.map((is) => (
                      <th key={is.year} className="text-right py-2">
                        {is.year}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  <tr className="border-b border-border/40">
                    ...
                    {fundamentals.data.incomeStatement.map((is) => (
                      <td key={is.year} className="text-right text-text-primary">
                        {is.revenue.toLocaleString()}
                      </td>
                    ))}
                  </tr>
                  <tr className="border-b border-border/40">
                  ...
                  <tr className="border-b border-border/40">
                    ...
                    {fundamentals.data.incomeStatement.map((is) => (
                      <td key={is.year} className="text-right text-green-400 font-extrabold">
                        {is.netIncome.toLocaleString()}
                      </td>
                    ))}
                  </tr>
                </tbody>
