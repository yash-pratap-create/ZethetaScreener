...
// ── Sector shock map (correlated movements within sectors) ────────────────────
...
// ── In-memory price state (not react state) ───────────────────────────────────
...
export function useRealtimeUpdates() {
  const { applyBatchUpdate, setConnectionStatus, incrementReconnect, resetReconnect, tickHeartbeat } =
  ...
  const generateBatch = useCallback(() => {
    ...
    // Refresh sector shocks once per batch (correlated movements)
  ...
  const connect = useCallback(() => {
    ...
    // Initialize prices from universe on first connect
