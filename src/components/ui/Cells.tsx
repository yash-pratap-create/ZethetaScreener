...
export const ChangeCell = memo(
  ...
  (prev, next) =>
    ...
    prev.showAbsolute === next.showAbsolute,
);
export const VolumeCell = memo(
  function VolumeCell({ value, avgVolume }: { value: number; avgVolume?: number }) {
    ...
    return (
      <span
        ...
        style={{
          color: isHigh ? '#f59e0b' : 'var(--color-text-secondary)',
...
export const RSICell = memo(
  function RSICell({ value }: { value: number }) {
    const color = value > 70 ? '#ef4444' : value < 30 ? '#22c55e' : 'var(--color-text-secondary)';
  ...
  function SectorBadgeCell({ value }: { value: string }) {
    return (
      <span
        ...
        style={{ background: 'var(--color-bg-surface-3)', color: 'var(--color-text-muted)' }}
      >
        {value}
      </span>
    );
...
const CAP_COLORS: Record<string, string> = {
  ...
  'Mid Cap': '#818cf8',
  'Small Cap': '#a78bfa',
  'Micro Cap': 'var(--color-text-muted)',
};
...
export const RoeCell = memo(
  function RoeCell({ value }: { value: number }) {
    return (
      <span
        className="font-mono text-sm tabular-nums"
        style={{ color: value >= 15 ? '#22c55e' : value < 0 ? '#ef4444' : 'inherit' }}
      >
        {value.toFixed(1)}%
      </span>
    );
...
export const PromoterCell = memo(
  function PromoterCell({ value }: { value: number }) {
    return (
      <span
        className="font-mono text-sm tabular-nums"
        style={{ color: value >= 50 ? '#22c55e' : 'inherit' }}
      >
        {value.toFixed(1)}%
      </span>
    );
...
export const DebtCell = memo(
  function DebtCell({ value }: { value: number }) {
    return (
      <span
        className="font-mono text-sm tabular-nums"
        style={{ color: value > 2 ? '#ef4444' : 'inherit' }}
      >
        {value.toFixed(2)}
      </span>
    );
...
export const MACD_COLORS: Record<
  string,
  {
    bg: string;
    text: string;
...
export const VOL_COLORS: Record<
  string,
  {
    bg: string;
    text: string;
