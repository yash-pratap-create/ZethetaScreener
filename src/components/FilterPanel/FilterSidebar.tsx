...
import {
  Sector, MACDSignal, IndexMembership, MarketCapCategory,
  BollingerPosition, VolumeVsAvg, PriceVsSMA,
} from '@/types/stock';
import { SECTORS } from '@/constants/SECTORS';

// ── A6.1: 30+ Filter Criteria organised by category ──────────────────────────
// Fundamentals (12) + Market Data (7) + Technical (10) + Classification (4) + Custom (2) = 35+

const NUMERIC_FIELDS: { id: keyof import('@/types').Stock; label: string; category: string }[] = [
  // Fundamentals — 12 criteria
  ...
  // Technical — numeric range criteria
...
const INDICES: IndexMembership[] = ['NIFTY50', 'NIFTY100', 'NIFTY500', 'SENSEX', 'MIDCAP150', 'SMALLCAP250'];
...
type SelectField =
  | 'sector' | 'macdSignal' | 'indexMembership' | 'marketCapCategory'
  | 'bollingerPosition' | 'volumeVsAvg' | 'industry'
  | 'priceVsSMA50Signal' | 'priceVsSMA200Signal';

const SELECT_FIELD_CONFIG: Record<SelectField, { label: string; options: string[] }> = {
  sector:              { label: 'Sector',              options: SECTORS },
  industry:            { label: 'Industry',            options: [] }, // derived from sector
...
function NumericFilterRow({
  ...
  rule: Extract<FilterRule, { type: 'numeric' }>;
  index: number;
  ...
  return (
    <div className="flex flex-col gap-2 p-3 rounded-lg border border-border bg-surface-2" role="group" aria-label={`Numeric filter rule ${index + 1}`}>
      <div className="flex items-center justify-between gap-2">
        <div className="flex flex-col gap-1 flex-1">
          <label htmlFor={`${idPrefix}-field`} className="text-[10px] uppercase font-bold text-text-muted">Metric</label>
          <select
            id={`${idPrefix}-field`}
            ...
            style={{ background: 'var(--color-bg-surface-3)', color: 'var(--color-text-primary)', borderColor: 'var(--color-border)' }}
          >
            {NUMERIC_FIELDS.map((f) => (
              <option key={f.id as string} value={f.id as string}>{f.label}</option>
            ))}
          </select>
        </div>
        <button
      ...
      <div className="flex items-end gap-2">
        <div className="flex flex-col gap-1">
          <label htmlFor={`${idPrefix}-op`} className="text-[10px] uppercase font-bold text-text-muted">Condition</label>
          <select
            id={`${idPrefix}-op`}
            value={rule.operator}
            onChange={(e) => onUpdate(index, { ...rule, operator: e.target.value as 'gt' | 'gte' | 'lt' | 'lte' | 'eq' | 'between' })}
            className="text-xs rounded px-2 py-1"
            style={{ background: 'var(--color-bg-surface-3)', color: 'var(--color-text-primary)', border: '1px solid var(--color-border)' }}
          >
        ...
        <div className="flex flex-col gap-1 flex-1">
          <label htmlFor={`${idPrefix}-val`} className="text-[10px] uppercase font-bold text-text-muted">Value</label>
          <input
            id={`${idPrefix}-val`}
            type="number"
            value={rule.value}
        ...
        {rule.operator === 'between' && (
          <>
            ...
            <div className="flex flex-col gap-1 flex-1">
              <label htmlFor={`${idPrefix}-val2`} className="text-[10px] uppercase font-bold text-text-muted">Max Value</label>
              <input
                id={`${idPrefix}-val2`}
                type="number"
                value={rule.value2 ?? ''}
                onChange={(e) => onUpdate(index, { ...rule, value2: parseFloat(e.target.value) || 0 })}
                className="w-full text-xs rounded px-2 py-1 font-mono"
                style={{ background: 'var(--color-bg-surface-3)', color: 'var(--color-text-primary)', border: '1px solid var(--color-border)' }}
...
function SelectFilterRow({
  ...
  rule: Extract<FilterRule, { type: 'select' }>;
  index: number;
  onUpdate: (i: number, r: FilterRule) => void;
  onRemove: (i: number) => void;
}) {
  ...
  return (
    <div className="flex flex-col gap-2 p-3 rounded-lg border border-border bg-surface-2" role="group" aria-label={`Select filter rule for ${label}`}>
      <div className="flex items-center justify-between">
        <label id={`${idPrefix}-select-label`} className="text-xs font-medium text-text-secondary">
          {label} <span className="text-text-muted font-normal text-[10px]">({selectedCountMsg})</span>
        </label>
        <button onClick={() => onRemove(index)}
          className="text-text-muted hover:text-red-400 transition-colors p-1" aria-label={`Remove select filter for ${label}`}>
          ✕
        </button>
      </div>
...
function BooleanFilterRow({
  ...
  rule: Extract<FilterRule, { type: 'boolean' }>;
  index: number;
  onUpdate: (i: number, r: FilterRule) => void;
  onRemove: (i: number) => void;
}) {
  ...
  return (
    <div className="flex items-center justify-between p-3 rounded-lg border border-border bg-surface-2" role="group" aria-label={`Boolean filter rule for ${label}`}>
      <label id={`${idPrefix}-label`} className="text-xs font-medium text-text-secondary">{label}</label>
      <div className="flex items-center gap-2">
        ...
        <button onClick={() => onRemove(index)}
          className="text-text-muted hover:text-red-400 transition-colors p-1" aria-label={`Remove boolean filter for ${label}`}>
          ✕
        </button>
      </div>
    </div>
...
function FilterRuleRow({
  ...
  if (rule.type === 'numeric') return <NumericFilterRow rule={rule} index={index} onUpdate={onUpdate} onRemove={onRemove} />;
  if (rule.type === 'select')  return <SelectFilterRow  rule={rule} index={index} onUpdate={onUpdate} onRemove={onRemove} />;
  if (rule.type === 'boolean') return <BooleanFilterRow rule={rule} index={index} onUpdate={onUpdate} onRemove={onRemove} />;
  return null;
}
...
export function FilterSidebar() {
  const {
    activeGroup, activePresetId, savedFilters,
    addRule, removeRule, updateRule, clearAllRules, setLogic, applyPreset,
    applySavedFilter, saveCurrentFilter, deleteSavedFilter
  } = useFilterStore();
  ...
  const handleAddRule = useCallback(() => {
    switch (addMode) {
      case 'numeric':
        ...
        break;
      // Classification select filters
      case 'sector':
        addRule({ type: 'select', field: 'sector', values: [] }); break;
      case 'industry':
        addRule({ type: 'select', field: 'industry', values: [] }); break;
      case 'marketCapCategory':
        addRule({ type: 'select', field: 'marketCapCategory', values: [] }); break;
      case 'indexMembership':
        addRule({ type: 'select', field: 'indexMembership', values: [] }); break;
      // Technical select filters
      case 'macdSignal':
        addRule({ type: 'select', field: 'macdSignal', values: ['Bullish'] }); break;
      case 'priceVsSMA50Signal':
        addRule({ type: 'select', field: 'priceVsSMA50Signal', values: ['Above'] }); break;
      case 'priceVsSMA200Signal':
        addRule({ type: 'select', field: 'priceVsSMA200Signal', values: ['Above'] }); break;
      case 'bollingerPosition':
        addRule({ type: 'select', field: 'bollingerPosition', values: [] }); break;
      case 'volumeVsAvg':
        addRule({ type: 'select', field: 'volumeVsAvg', values: [] }); break;
      // Custom boolean filters
      case 'watchlistOnly':
        addRule({ type: 'boolean', field: 'isWatched', value: true }); break;
      case 'recentlyUpdated':
        addRule({ type: 'boolean', field: 'isActive', value: true }); break;
    }
  }, [addMode, addRule]);
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex items-center gap-2">
          <svg width="10" height="10" viewBox="0 0 24 24" fill="currentColor" style={{ color: 'var(--color-accent-primary)', flexShrink: 0 }}>
          ...
          {FILTER_PRESETS.map((preset) => (
            <button
              ...
              style={{
                ...
                color: activePresetId === preset.id ? 'var(--color-accent-primary)' : 'var(--color-text-muted)',
                background: activePresetId === preset.id ? 'rgba(30,136,229,0.10)' : 'transparent',
                borderLeft: activePresetId === preset.id ? '2px solid var(--color-accent-primary)' : '2px solid transparent',
      ...
      <div className="flex-1 flex flex-col gap-2 px-3 py-3 overflow-y-auto">
        <div className="flex items-center justify-between mb-1">
          <p className="text-xs text-text-muted uppercase tracking-wider font-semibold">Custom Filters</p>
          {activeGroup.rules.length > 1 && (
            <div className="flex border border-border rounded-md overflow-hidden text-xs">
              <button
                ...
                style={activeGroup.logic === 'AND' ? { background: 'var(--color-accent-primary)' } : {}}
              >
                AND
              </button>
              <button
                ...
                style={activeGroup.logic === 'OR' ? { background: 'var(--color-accent-primary)' } : {}}
              >
                OR
              </button>
            </div>
        ...
        {/* Live announcer wrapper */}
        <div role="log" aria-live="polite" className="flex flex-col gap-2">
          {activeGroup.rules.map((rule, i) => (
            <FilterRuleRow key={i} rule={rule} index={i} onUpdate={updateRule} onRemove={removeRule} />
          ))}
        </div>
        ...
        <div className="flex flex-col gap-1.5 mt-2 p-3 rounded-lg border border-dashed border-border bg-surface-1">
          <label htmlFor="add-filter-select" className="text-[10px] uppercase font-bold text-text-muted">Add Filter Criteria</label>
          <div className="flex gap-1.5">
            <select
              ...
              style={{ background: 'var(--color-bg-surface-2)', color: 'var(--color-text-primary)', borderColor: 'var(--color-border)' }}
            >
        ...
        {activeGroup.rules.length > 0 && (
          <div className="mt-2 pt-2 border-t border-border">
            ...
            <div className="flex items-center gap-1.5 mt-2">
              <label htmlFor="save-filter-name" className="sr-only">Name for saved filter</label>
              <input
                id="save-filter-name"
