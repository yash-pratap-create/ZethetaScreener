...
function IndexChip({ label, value, change }: { label: string; value: number; change: number }) {
  ...
  return (
    <div className="market-chip">
      ...
      <span
        style={{ color: isUp ? 'var(--color-positive)' : 'var(--color-negative)', fontSize: 10 }}
      >
...
function MarketIndices() {
  ...
  const indices = useMemo(() => {
    ...
    return [
      { label: 'NIFTY 50', value: 24315, change: avgChange(nifty50) },
      { label: 'SENSEX', value: 80218, change: avgChange(sensex) },
      { label: 'NIFTY MID', value: 12847, change: avgChange(midcap) },
    ];
  }, [stocks, priceUpdates]);
      ...
      {status === 'connected' && (
        <>
          ...
          <span className="mono-num" style={{ color: 'var(--color-text-muted)', fontSize: 9 }}>
            {ups}
            <span style={{ color: 'var(--color-text-dim)' }}>UPS</span>
          </span>
          {latency > 0 && (
            <>
              <span style={{ color: 'var(--color-border-strong)', fontSize: 9 }}>│</span>
